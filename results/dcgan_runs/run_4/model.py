...
class DCGAN(object):
    def __init__(self, sess, cfg, input_height=108, input_width=108, crop=True,
        ...
        if cfg.generator()['batch_norm']:
            print('dcgan: batch_norm true')
        ...
        if self.dataset_name == 'mnist':
            ...
            self.data = glob(os.path.join("/home/jester/data/dcgan/data", self.dataset_name, self.input_fname_pattern))
            print('dataset ' + '/home/jester/data/dcgan/data/' + self.dataset_name)
        ...
        for epoch in xrange(config.epoch):
            ...
            if config.dataset == 'mnist':
                ...
                self.data = glob(os.path.join(
                    "/home/jester/data/dcgan/data", config.dataset, self.input_fname_pattern))
            ...
            for idx in xrange(0, batch_idxs):
                if config.dataset == 'mnist':
                    ...
                    if self.grayscale:
                        self.none_ = np.array(batch).astype(np.float32)[:, :, :, None]
                        batch_images = self.none_
                    else:
    ...
    def discriminator(self, image, y=None, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            if not self.y_dim:
                print ('discriminator: if not y_dim')
                if self.cfg.discriminator()['lrelu']:
                    print('discriminator: lrelu')
                    h0 = lrelu(conv2d(image, self.df_dim, name='d_h0_conv'))
                    h1 = lrelu(self.d_bn1(conv2d(h0, self.df_dim * 2, name='d_h1_conv')))
                    h2 = lrelu(self.d_bn2(conv2d(h1, self.df_dim * 4, name='d_h2_conv')))
                    h3 = lrelu(self.d_bn3(conv2d(h2, self.df_dim * 8, name='d_h3_conv')))
                elif self.cfg.discriminator()['relu']:
                    print('discriminator: relu')
                ...
                elif self.cfg.discriminator()['tanh']:
                    print('discriminator: tanh')
                    h0 = tf.nn.tanh(conv2d(image, self.df_dim, name='d_h0_conv'))
                    h1 = tf.nn.tanh(self.d_bn1(conv2d(h0, self.df_dim * 2, name='d_h1_conv')))
                    h2 = tf.nn.tanh(self.d_bn2(conv2d(h1, self.df_dim * 4, name='d_h2_conv')))
    ...
    def generator(self, z, y=None):
        with tf.variable_scope("generator") as scope:
            if not self.y_dim:
                ...
                if self.cfg.generator()['relu']:
                    print('generator: relu')
                    h0 = tf.nn.relu(self.g_bn0(self.h0))
                elif self.cfg.generator()['lrelu']:
                    print('generator: lrelu')
                    h0 = lrelu(self.g_bn0(self.h0))
                elif self.cfg.generator()['tanh']:
                    print('generator: relu')
    ...
    def sampler(self, z, y=None):
        with tf.variable_scope("generator") as scope:
            ...
            if not self.y_dim:
                ...
                h2 = tf.nn.relu(
                    self.g_bn2(deconv2d(h1, [self.batch_size, s_h2, s_w2, self.gf_dim * 2], name='g_h2'), train=False))
                h2 = conv_cond_concat(h2, yb)
